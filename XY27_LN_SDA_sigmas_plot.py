...
for file_name in range(len(file_list)):
    ...
    print(dane[5])
    dice.append(dane[5])
    # sigma_1_x = dane[0].astype(np.float)
    # sigma_2_x = dane[1].astype(np.float)
    # sdar_x = dane[2].astype(np.float)
    # sdat_x = dane[3].astype(np.float)
    # dice_x = dane[5].astype(np.float)
...
# yline2 = sigma_2
# #
# ax.scatter(yline,xline, zline,label='LN Discnormalise XY27' )
# ax.scatter(yline2,xline2, zline2,label='Sigma1(x), Sigma2(y)' )
# ax.set_xlabel('SDAr')
# ax.set_ylabel('SDAt')
